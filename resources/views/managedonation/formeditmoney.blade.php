    ...
    <div class="md:col-span-1">
        <div class="container" style="display: flex; justify-content: center;">
            <div class="w-full sm:max-w-lg mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg" >
                <p class="h1 mt-2 mb-4 block text-xl font-semibold text-black" >Edit Donasi</p> <!-- Nama Campaign Yang Dipilih -->
                ...
                <form method="POST" action="{{ route('donation.summary.edit') }}" style="margin: 0 auto;">
                    ...
                    <input type="hidden" name="id_donasi" value="{{ $formdonation->id }}">
                    <!-- Input Nominal Donasi -->
                    <div class="mb-4 mt-6">
                        <label class="block font-medium text-sm text-gray-700" for="nominal">Nominal Donasi</label>
                        <input class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full h-16 text-lg" id="nominal" type="number" name="nominal" required value="{{ $formdonation->moneyDonations->first()->nominal }}">
                    </div>
                    ...
                    <div class="mb-4 mt-6">
                        ...
                        <select class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" id="metode_pembayaran" name="metode_pembayaran" required>
                            <option value="{{ $formdonation->moneyDonations->first()->nama_bank }}">{{ $formdonation->moneyDonations->first()->nama_bank }}</option>
                    ...
                    <div class="mb-4">
                        ...
                        <input class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" id="nama_pemilik" type="text" name="nama_pemilik" value="{{ $formdonation->moneyDonations->first()->nama_pemilik }}" required>
                    </div>
                    <!-- Input Nomor Rekening -->
                    <div class="mb-6">
                        <label class="block font-medium text-sm text-gray-700" for="nomor_rekening">Nomor Rekening</label>
                        <input class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" id="nomor_rekening" type="text" name="nomor_rekening" value="{{ $formdonation->moneyDonations->first()->nomor_rekening }}" required>
                    </div>
                    <!-- Input Pesan -->
                    <div class="mb-4 mt-6">
                    ...
                    {{-- <div class="mb-4">
                        <label class="inline-flex items-center">
                            <input class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" type="checkbox"  name="syarat_dan_ketentuan" value="{{ $formdonation->syarat_ketentuan }}" required>
