...
<div class="grid h-fit w-full grid-flow-row">
    ...
    <div class="col-span-3 mt-16 lg:mt-4">
        ...
        <form action="{{ route('admin.berita.update', ['id' => $news->id]) }}" method="POST"
            enctype="multipart/form-data" class="ml-14 mr-14">
            @csrf
            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700">Judul</label>
                <input type="text" name="title" id="title" class="mt-1 block w-full" value="{{$news->title}}">
            </div>
            <div class="mb-4">
                <label for="content" class="block text-sm font-medium text-gray-700">Konten</label>
                <textarea name="content" id="content" rows="5" class="mt-1 block w-full">{{$news->content}}</textarea>
            </div>
            <div class="mb-4">
                <label for="tanggal" class="block text-sm font-medium text-gray-700">Tanggal</label>
                <input type="date" name="release_date" id="tanggal" class="mt-1 block w-1/3"
                    value="{{$news->release_date}}">
            </div>
            <div class="mb-4">
                <label for="foto" class="block text-sm font-medium text-gray-700">Gambar</label>
                <input type="file" name="image" id="foto" class="mt-1 block w-full" accept="image/*">
            </div>
            <div class="mb-4">
